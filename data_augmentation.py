...
def show_pic(img, label, bboxes=None):
    ...
    for i in range(len(bboxes)):
        ...
        cv2.putText(img, label, (int(x_min), int(y_min)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    cv2.namedWindow('pic', 0)  # 1表示原图
    cv2.moveWindow('pic', 0, 0)
    cv2.resizeWindow('pic', 512,512)  # 可视化的图片大小
    cv2.imshow('pic', img)
    cv2.waitKey(0)
    ...
    def dataAugment(self, img, bboxes):
        ...
        while change_num < 1:   #默认至少有一种数据增强生效
            ...
            if random.random() > self.rotation_rate:    #旋转
                ...
                angle = random.sample([2,4,-2,-4],1)[0]
...
if __name__ == '__main__':
    ...
    def save_annotation(file, img_path, bboxes, label):
        _bboxes = [str(round(i)) for i in bboxes[0]]
        _bboxes.append(str(label))
        bbox_and_label = ",".join(_bboxes)
        line = img_path + ".png " + bbox_and_label + "\n"
        try:
            file.write(line)
            print("Save image ", img_path)
        except:
            print("Errors occur while writing to file")
    ...
    new_bbox_path = './train12.txt'
    new_img_path = '../dataset/train12/'
    ...
    for line in lines:
        ...
        save_annotation(trainFile, img_path, bboxes, label)

        while cnt < need_aug_num:
            auged_img, auged_bboxes = dataAug.dataAugment(img, bboxes)
            cnt +=1
            # show_pic(auged_img, class_name, auged_bboxes)
            auged_img_path = img_path + str(cnt)
            cv2.imwrite(auged_img_path+".png", auged_img)
            save_annotation(trainFile, auged_img_path, auged_bboxes, label)
