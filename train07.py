...
gpuconfig.gpu_options.per_process_gpu_memory_fraction=0.50
...
def _main():
    annotation_path = 'train07.txt'
    log_dir = 'logs/007new/'
    classes_path = 'model_data/train06_classes.txt'
    anchors_path = 'model_data/train07_anchors.txt'
    ...
    reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=3, verbose=1, min_lr=1e-45)
    early_stopping = EarlyStopping(monitor='val_loss', min_delta=0, patience=100, verbose=1)
    csv_logger1 = CSVLogger(log_dir + "log1.csv")
    csv_logger2 = CSVLogger(log_dir + "log2.csv")
    csv_logger3 = CSVLogger(log_dir + "log3.csv")
